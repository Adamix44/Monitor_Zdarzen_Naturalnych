...
/**
 * Tłumaczy angielskie nazwy kategorii zdarzeń zwracane przez NASA EONET API na nazwy w języku
 * polskim.
...
/**
 * Główna klasa ViewModel w architekturze MVVM dla głównego ekranu aplikacji. Odpowiada za:
 * 1.Przechowywanie i zarządzanie stanem interfejsu użytkownika 2.Pośredniczenie między warstwą
 * danych ([EventRepository]) a widokiem (Jetpack Compose). 3.Obsługę logiki, takiej jak filtracja
 * danych
 *
 * Stosuje zasadę enkapsulacji. Właściwości modyfikowalne (_allEvents, _isLoading itp.) są
 * prywatnymi obiektami typu [MutableLiveData], natomiast widok ma dostęp wyłącznie do ich
 * niemodyfikowalnych, publicznych odpowiedników typu [LiveData].
 */
class MainViewModel(application: Application) : AndroidViewModel(application) {

    /** repozytorium służące do komunikacji z API NASA. */
    private val repository = EventRepository()
    private val alarmPreferences = AlarmPreferences(application)

    /** kontener przechowujący nieprzefiltrowaną listę wszystkich pobranych z NASA zdarzeń. */
    ...
    val availableCategories: LiveData<List<String>>
    ...
    /**
     * Zmienia zakres dni Ta zmiana wymaga pobrania nowych danych z NASA, dlatego wywoływana jest
     * metoda [loadEvents].
    ...
    /**
     * Włącza alarm z podanym zasięgiem w kilometrach. radiusKm = 0 oznacza monitorowanie całej
     * planety.
     */
    fun enableAlarm(radiusKm: Int) {
        ...
        // zapamiętuje zdarzenia, żeby nie powiadamiać o już istniejących
    ...
    /** Planuje sprawdzanie zdarzeń co 15 minut */
    private fun scheduleAlarmWorker() {
        val constraints =
                Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build()

        val workRequest =
                PeriodicWorkRequestBuilder<EventAlarmWorker>(15, TimeUnit.MINUTES)
    ...
    // funkcje sieciowe
    /**
     * pobieranie danych z NASA EONET API. Jeśli użytkownik opuści ekran i ViewModel zostanie
     * zniszczony, wszelkie trwające żądania sieciowe w tym zakresie zostaną automatycznie
     * anulowane.
     */
    fun loadEvents() {
    ...
    /**
     * filtruje pełną listę zdarzeń przechowywaną w buforze [_allEvents] na podstawie wybranej
     * kategorii [_selectedCategory] i publikuje wynik do [_events].
     */
