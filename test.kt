...
/**
 * Parsuje strukturę JSON z koordynatami geograficznymi dostarczanymi przez NASA EONET API. NASA
 * EONET API zwraca współrzędne w formacie [Długość geograficzna, Szerokość geograficzna].
 * @param coords Obiekt JsonArray reprezentuje surowe współrzędne pobrane z API.
 * @return Para (Szerokość, Długość) do użycia w Google Maps.
 */
fun parseLatLng(coords: JsonArray): Pair<Double, Double>? {
    try {
        ...
        if (coords.size() == 2 && coords[0].isJsonPrimitive) {
            ...
            // Dla Google Maps i standardowej pary zwracamy [Latitude, Longitude], stąd zamiana
            // kolejności: coords[1], coords[0].
            ...
            // Gdy dotrzemy do najgłębszego poziomu, sprawdzamy czy mamy przynajmniej 2 elementy
            // liczbowe
 ...
 */
fun main() {
    // Przykładowy ciąg JSON reprezentujący punkt geograficzny (współrzędne w formacie EONET: [lng,
    // lat])
